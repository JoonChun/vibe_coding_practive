...
# 마스터 파일이 잘린 채 정상 zip 으로 도착하면 행 수만 줄어든 채 조용히 성공하는데,
# 그때 "없어진 코드 = 상폐"를 그대로 적용하면 멀쩡한 전 종목이 상폐로 찍힌다.
# 아래 두 조건을 모두 통과할 때만 상폐 표시를 실행한다(실패하면 upsert 만 하고
# 상폐 판정은 다음 갱신으로 미룬다 — 데이터는 최신화되고 판정만 유예된다).
STOCK_MASTER_MIN_ROWS_PER_MARKET = 1000   # 시장별 절대 하한(실측 KOSPI 1,788 / KOSDAQ 1,776)
STOCK_MASTER_MIN_RATIO = 0.95             # 기존 상장 종목 수 대비 하한(실제 상폐는 연 수십 건)
