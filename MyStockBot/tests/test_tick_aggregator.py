...
@pytest.mark.asyncio
async def test_consume_loop_ignores_non_tick_events(monkeypatch):
...
def test_compute_progressing_bars_includes_zero_volume_bar_and_excludes_incomplete_day_bar():
    # forming 은 완결(volume=0 포함)인데 day_bar 는 아예 없음(None) — "1d" 만 빠지고
    # 나머지 tf(1m 포함 전부)는 forming 단일 항목만으로도 완결 집계돼 나와야 한다.
    ...
    assert tfs == set(tick_agg._ALL_TFS) - {"1d"}
...
def test_compute_progressing_bars_multi_code_and_multi_tf():
    ...
    assert tfs_seen == set(tick_agg._ALL_TFS)
