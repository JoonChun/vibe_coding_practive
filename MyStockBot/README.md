...
> 설계 결정·함정·작업 규약은 [ARCHITECTURE.md](ARCHITECTURE.md) 참조.
...
>
> **✅ Discord 는 2026-08-23 사용자 네트워크에서 실발송이 확인됐다**
> (`scripts/diagnose_alerts.py --probe` → `✓ discord: 발송 성공`). 위 규격 — 특히
> `?wait=true` 를 붙여야 저장 여부를 알 수 있다는 점 — 이 실제로 통한다는 뜻이다.
> **Slack 은 여전히 미검증**(사용자가 설정하지 않음).
...
- **Slack 실발송 미검증** — 개발 환경에서 벤더 도메인이 차단돼 규격은 1차 소스(공식 SDK
  소스)로만 확인했다. `scripts/diagnose_alerts.py --probe` 로 각자 확인해야 한다.
  (**Discord 는 2026-08-23 사용자 네트워크에서 실발송 확인됨.**)
