...
| `/alerts` | 알림 진단 | 인식된 채널·발송 조건·기준선 확인 + 테스트 발송(읽기 전용) |
...
4개 화면의 마운트·핵심 문구·**탭바 줄 수**·콘솔 에러·테스트 발송 왕복을 확인합니다.
탭바 줄 수 검사가 있는 이유는 실제로 그 결함을 겪었기 때문입니다 — 탭을 4개로 늘렸을 때
`repeat(3, 1fr)` 하드코딩 때문에 2줄로 깨졌고, tsc·pytest 는 전부 통과했습니다.
