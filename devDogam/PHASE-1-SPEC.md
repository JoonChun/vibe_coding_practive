...
- [ ] 모바일 반응형 (기본: desktop 우선)
- [ ] **dev 서버 + 브라우저 콘솔 0 에러**: `npm run dev` 띄운 상태에서 Chrome DevTools 콘솔 에러 0건 확인. 빌드·테스트 통과만으론 부족 — 런타임 hydration 에러나 무한 루프는 빌드 시 안 잡힘. (*사유*: 임시 hotfix 3건이 모두 빌드 통과 후에야 사용자 첫 로드에서 드러남)
- [ ] **메인 세션 1회 작업 트리거 → 시각 변화 확인**: 메인 Claude 세션에서 작업 1건을 트리거하여 (a) 매니저 캐릭터 점등, (b) 말풍선 1회 이상 출현, (c) 두루마리 제목 변화, (d) 도제 등장(해당 작업 도제 영역일 때) 확인. 정적 시험만으론 통합 결함을 못 잡음. (*사유*: 임시 hotfix 3건이 모두 빌드 통과 후에야 사용자 첫 로드에서 드러남)
- [ ] **narrow viewport (9:16) 정합성**: 모바일 인스타 reels 9:16 세로 캡처 환경에서 어전 도열·매니저·옥좌 위치 정합, 반응형 깨짐 0건 확인. (현재 desktop 64×64px 기준이나, reels 콘텐츠 평가 시 재점검)
