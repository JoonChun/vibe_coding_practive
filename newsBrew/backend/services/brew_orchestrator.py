...
async def run_brew_pipeline():
    ...
    try:
        ...
        if not keywords:
            ...
            await manager.broadcast("DONE", "브루잉 완료!")
            return
        ...
        for kw in keywords:
            if not kw.is_active:
                await manager.broadcast("INFO", f"⏭️ [{kw.word}] 비활성화 — 건너뜀")
                continue
            await manager.broadcast("INFO", f"🔍 [{kw.word}] 뉴스 수집 중...")
            try:
                news_provider = settings.get("news_provider", "serpapi")
                hours = kw.time_range_hours or 24
                date_range = f"d{max(1, hours // 24)}"
                news_items = await collect_news(
                    keyword=kw.word,
                    exclude_words=kw.exclude_words or "",
                    api_key=settings.get("serpapi_key", ""),
                    max_results=kw.max_results or 15,
                ...
                ai_key = settings.get("gemini_api_key", "") if ai_provider == "gemini" else settings.get("openai_key", "")
                brewed = await brew_articles(
                    ...
                    gemini_model=settings.get("gemini_model", "gemini-2.0-flash-preview-04-17"),
                )
        ...
        try:
            ...
            await manager.broadcast("ERROR", f"DB 저장 오류: {str(e)}")
        ...
        await manager.broadcast("ERROR", f"브루잉 파이프라인 오류: {str(e)}")
        await manager.broadcast("DONE", "브루잉 실패")
